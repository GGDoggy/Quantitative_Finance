...
def _simulation_time_step_tokens(time_step: float, time_step_token: str | None = None) -> tuple[str, ...]:
    tokens: list[str] = []
...
def _simulation_value_tokens(value: float | None, value_token: str | None = None) -> tuple[str, ...]:
    if value is None:
        return ()
    try:
...
def _matches_resolved_time(metadata_resolved_time: float | None, metadata_resolved_time_token: str | None, resolved_time: float | None, resolved_time_tokens: set[str]) -> bool:
    if resolved_time is None:
        return True
    if metadata_resolved_time is None:
        return resolved_time == DEFAULT_RESOLVED_TIME
    return metadata_resolved_time_token in resolved_time_tokens or metadata_resolved_time == resolved_time
...
def find_simulation_files(preprocessed_dir: Path, product_id: str, timestamp: str, time_step: float, time_step_token: str | None = None, resolved_time: float | None = None, resolved_time_token: str | None = None, algorithm_name: str | None = None) -> tuple[Path, ...]:
    ...
    for file_path in _iter_files(preprocessed_dir, ".npz"):
        ...
        if metadata.product_id != product_id or metadata.timestamp != timestamp:
            continue
        if not (metadata.time_step_token in time_step_tokens or metadata.time_step == time_step):
            continue
        if algorithm_name is not None and metadata.algorithm_name != algorithm_name:
            continue
        if not _matches_resolved_time(metadata.resolved_time, metadata.resolved_time_token, resolved_time, resolved_time_tokens):
            continue
